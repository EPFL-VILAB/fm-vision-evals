...
from taskit.utils.data import replace_images_in_prompt, save_images, draw_around_superpixel
...
def system_prompts_od(prompt_no: int, obj: str, independent: bool = False, mark_rectangle: bool = False) -> str:
    if prompt_no == 1 and independent:
        if mark_rectangle:
            system_prompt = f"""You are an advanced object detection model. You will be provided with an image marked with a red rectangle. Your task is to determine whether any part of the {obj} is present in the rectangle.\n\n""" +\
        ...
        system_prompt = f""" You are an advanced object detection model. You will be provided with 9 grid cells (indexed from 1 to 9). For each grid cell, you need to output whether any part of the {obj} is present in the cell or not ("yes" if it is, "no" if it isn't).\n\n""" +\
        ...
        if mark_rectangle:
            system_prompt = """You are an advanced object detection model. You will be provided with an image marked with a red rectangle. Your task is to determine whether any part of the specified object is present in the rectangle.\n""" +\
        ...
        system_prompt = """You are an advanced object detection model tasked with identifying the presence of a specific object in a set of grid cells. Your goal is to analyze each cell and determine whether any part of the target object is present.\n""" +\
                        """The object you are looking for is:\n""" +\
                        f"""<object>{obj}</object>\n\n""" +\
        ...
        if mark_rectangle:
            system_prompt = """You are an advanced object detection model with exceptional analytical capabilities. Your task is to detect the presence of a specified object within the rectangle marked in red.\n\n""" +\
                """**Target Object**:\n""" +\
                f"""<object>{obj}</object>\n""" +\
                """\n""" +\
                """**Grid Information**:\n""" +\
        ...
        system_prompt = """You are an advanced object detection model with exceptional analytical capabilities. Your task is to detect the presence of a specified object within a set of grid cells.\n\n""" +\
                        """**Target Object**:\n""" +\
                        f"""<object>{obj}</object>\n""" +\
                        """\n""" +\
                        """**Grid Information**:\n""" +\
        ...
        if mark_rectangle:
            system_prompt = """You are an advanced object detection model. Your task is to detect the presence of a specified object within the marked rectangle.\n\n""" +\
                        """**Target Object**:\n""" +\
                        f"""<object>{obj}</object>\n""" +\
                        """\n""" +\
        ...
        system_prompt = """You are an advanced object detection model. Your task is to detect the presence of a specified object within a set of grid cells.\n\n""" +\
                        """**Target Object**:\n""" +\
                        f"""<object>{obj}</object>\n""" +\
                        """\n""" +\
                        """**Grid Information**:\n""" +\
...
def system_prompts_od_naive(prompt_no: int, obj: str, normalization: int) -> str:
    ...
    if prompt_no == 1:
        system_prompt = f""" Locate the {obj} and represent the location of the region. Regions are represented by [x1, y1, x2, y2] coordinates. x1 x2 """ +\
        ...
        system_prompt = """You are an AI assistant tasked with detecting objects in images. Your goal is to locate a specific object within an image description and represent its location using normalized coordinates.\n\n""" +\
        ...
        system_prompt = """Your task is to precisely locate the object described below in the given image and represent its position using a normalized bounding box.\n\n""" +\
                f"""<object>{obj}</object>\n\n""" +\
        ...
        system_prompt = """You are tasked with identifying and locating the following object within an image:\n""" +\
                        f"""<object>{obj}</object>\n\n""" +\
                        """### Your Objective:\n""" +\
...
def json_schema_naive(prompt_no: int, obj: str, normalization: int, model: str):
    if model == 'gpt-4o-2024-08-06':
        if prompt_no in [3, 4]:
            json_schema, json_properties = {}, {}
            json_properties["reasoning_steps"] = {"type": "array", "items": {"type": "string"}, "description": "The step-by-step reasoning process leading to the final conclusion."}
            json_properties["coordinates"] = {"type": "array", "items": {"type": "number"}, "description": f"Coordinates of the bounding box for the {obj}. [x1, y1, x2, y2] normalized into 0 to {normalization}."}
            json_schema["name"] = "schema"
            json_schema["strict"] = True
            json_schema["schema"] = {"type": "object", "properties": json_properties, "required": ["reasoning_steps", "coordinates"], "additionalProperties": False}
        elif prompt_no == 5:
            json_schema, json_properties = {}, {}
            json_properties["coordinates"] = {"type": "array", "items": {"type": "number"}, "description": f"Coordinates of the bounding box for the {obj}. Normalized into 0 to {normalization}."}
            json_schema["name"] = "schema"
            json_schema["strict"] = True
            json_schema["schema"] = {"type": "object", "properties": json_properties, "required": ["coordinates"], "additionalProperties": False}
        else:
            json_schema, json_properties = {}, {}
            json_properties["coordinates"] = {"type": "array", "items": {"type": "number"}, "description": f"Coordinates of the bounding box for the {obj}. [x1, y1, x2, y2] normalized into 0 to {normalization}."}
            json_schema["name"] = "schema"
...
def full_prompt_detect_independent(prompt_no: int, obj: str, model: str, location: str, no_context: bool, mark_rectangle: bool) -> str:
    messages = []
    ...
    for i in range(rows):
        for j in range(cols):
            if mark_rectangle:
                ...
                segment_array[y_positions[i]:y_positions[i + 1], x_positions[j]:x_positions[j + 1]] = 1
                img_list = [draw_around_superpixel(img, segment_array, 1, 'rectangle', rectangle_width=1)]
            elif no_context:
                img_list = [img.crop((x_positions[j], y_positions[i], x_positions[j + 1], y_positions[i + 1]))]
            else:
                img_list = [img, img.crop((x_positions[j], y_positions[i], x_positions[j + 1], y_positions[i + 1]))]
            full_prompt = full_prompt_detect_independent(prompt_no, obj, model.name, locations[cols * i + j], no_context, mark_rectangle) if not prompt else prompt
            full_prompt = replace_images_in_prompt(full_prompt, img_list)

            resp_dict, tokens, err = model.send_message(full_prompt)
            if resp_dict is None or resp_dict['1'] == 'yes':
                np_img[i, j] = 1
    ...
    for img_idx, img in enumerate(imgs):
        ...
        for obj_idx, obj in enumerate(objects):
            ...
            for i in range(n_iters):
                if independent:
                    global_coords, tokens, done_zooming, err = independent_zoom(prompt, prompt_no, img_current, model, obj, global_coords, resolution, no_context, mark_rectangle)
                else:
...
@MFMWrapper.register_task('detect_naive')
def detect_naive(
    model: MFMWrapper,
    file_name: Union[List[str], str, List[Image.Image], Image.Image],
    prompt: Optional[Dict] = None,
    ...
    for img_idx, img in enumerate(imgs):
        ...
        coords = {"coords": {}}
        for obj in objects:
            full_prompt = full_prompt_naive(prompt_no, obj, normalization, model.name) if not prompt else prompt
            ...
            if len(coords['coords'][obj]) != 4:
                coords['coords'][obj] = [0, 0, width, height] if prompt_no != 5 else [0, 0, height, width]

            if prompt_no == 5:
                coords['coords'][obj] = [coords['coords'][obj][1], coords['coords'][obj][0], coords['coords'][obj][3], coords['coords'][obj][2]]

            coords['coords'][obj] = [[float(coords['coords'][obj][0] * width / normalization), float(coords['coords'][obj][1] * height / normalization)], [float(coords['coords'][obj][2] * width / normalization), float(coords['coords'][obj][3] * height / normalization)]]
