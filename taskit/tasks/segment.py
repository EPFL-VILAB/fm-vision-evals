...
def system_prompts_segment(
    prompt_no: int, seg_classes: list, batch_size: int, shape: str, classes_guessed=[]
):
    assert prompt_no in [1, 2, 3, 4, 5, 6], "Invalid prompt number."
    ...
    if prompt_no == 1:
        ...
        system_prompt = (
            f"""You are an advanced image classifier AI. Your task is to determine the semantic class of objects or backgrounds marked by {shape}s in an image. Each {shape} marks a part of an object or background, and you need to identify the class to which the entire object or background belongs.\n\n"""
        ...
        system_prompt = (
            f"""You are an expert semantic classifier. You will be given a set of {shape}s, each highlighting a segment of an object or background within an image. Your task is to identify the semantic class of the underlying object or background marked by each {shape}.\n\n"""
    ...
    elif prompt_no == 6 and (len(classes_guessed) > 0):
        system_prompt = (
            f"""You are an advanced image classifier AI. Your task is to determine the semantic class of objects or backgrounds marked by {shape}s in an image. Each {shape} marks a part of an object or background, and you need to identify the class to which the entire object or background belongs.\n\n"""
            + f"""<seg_classes>{seg_classes}</seg_classes>\nThese are the available semantic classes from which you must choose when classifying each {shape}.\n\n"""
            + f"""<classes_guessed>{classes_guessed}</classes_guessed>\nThese are the classes you have already guessed in previous segments. Use them if they are relevant to the current segment, otherwise, consider other classes from <seg_classes>.\n\n"""
...
def system_prompts_segment_sans_context(
    prompt_no: int, seg_classes: list, shape: str, classes_guessed=[], description=None
):
    ...
    if prompt_no == 1:
        ...
        system_prompt = (
            f"""You are an advanced image classifier AI. Your task is to determine the semantic class of objects or backgrounds marked by the {shape} in an image. The {shape} marks a part of an object or background, and you need to identify the class to which the entire object or background belongs.\n\n"""
        ...
        system_prompt = (
            f"""You are an advanced image classifier specializing in identifying semantic classes of objects or backgrounds marked by specific shapes in images. Your task is to analyze a full image with a red {shape}, and classify the semantic class of the object or background that the {shape} demarcates.\n"""
            + """1. First, examine the full image.\n"""
        ...
        system_prompt = (
            f"""You are an expert image classifier tasked with identifying the semantic classes of objects or backgrounds marked by specific shapes in images. Your job is to analyze a full image with a red {shape}, and classify the semantic class of the object or background that the {shape} marks.\n\n"""
            + """### Instructions:\n\n"""
            + """1. **Examine the Full Image**: Start by analyzing the entire image to understand the overall context.\n"""
        ...
        system_prompt = (
            f"""You are an expert semantic classifier. You will be given an image with a red {shape}, representing a segment of an object or background. Your task is to identify the semantic class of the underlying object or background marked by the {shape}.\n\n"""
            + """### Available Semantic Classes:\n"""
            + f"""{seg_classes}\n\n"""
            + """### Task Instructions:\n\n"""
...
def system_prompts_describe(prompt_no: int, previous_description=""):
    ...
    if len(previous_description) > 0:
        system_prompt = (
            """You will be provided with the crop of an image, taken from a larger scene. Your task is to describe the contents of the image in as much detail as possible. Do not hallucinate or make up details.\n\n"""
        ...
        system_prompt = (
            """You will be provided with an image. Your job is to describe the image in as much detail as possible. Do not hallucinate or make up details.\n\n"""
            + """Provide your response in the following JSON format:\n\n"""
            + """{\n"""
            + """  "description": "Your description here."\n"""
            + """}\n\n"""
...
def system_prompts_naive(
    prompt_no: int, seg_classes: list, batch_size: int, classes_guessed=[]
):
    if prompt_no == 1:
        system_prompt = (
    ...
    if model in OPENAI_MODELS:
        if model == "o1-2024-12-17":
            ...
            json_properties.update(
                {
                    str(k + 1): {
                        "type": "string",
                        "description": f"The semantic class of the object or background in the full image to which {shape} {k+1} belongs. The output must be one of the classes listed in the system prompt.",
                    }
            ...
            json_schema["schema"] = {
                ...
                "required": [str(k + 1) for k in range(batch_size)],
            ...
            json_schema["schema"] = {
                ...
                + [str(k + 1) for k in range(batch_size)],
                "additionalProperties": False,
            }
        ...
        json_properties.update(
            {
                str(k): {
                    "type": "string",
                    "description": f"The semantic class of the object or background in the full image which the numbered point {k} marks. The output must be one of the classes listed in the system prompt.",
                }
                for k in range(batch_size)
            }
        )
...
def full_prompt_segment(
    ...
    classes_guessed: list = [],
):
    messages = []

    system_prompt = system_prompts_segment(
        prompt_no, class_labels, batch_size, shape, classes_guessed
    )
        ...
        messages.append(
            {
                "role": "user",
                "content": [
                    {"type": "text", "text": f"Here is {shape} {j-(start_idx-1)}"},
                    {
                        "type": "image_url",
    ...
    messages.append(
        {"role": "user", "content": [{"type": "text", "text": user_prompt_3}]}
    )
    for j in range(start_idx, start_idx + batch_size):
        if j > n_unique_segments:
            break
        messages.append(
            {
                ...
                "content": [
                    {
                        "type": "text",
                        "text": f"Here is the context for {shape} {j-(start_idx-1)}",
                    },
                    {
                        "type": "image_url",
                        "image_url": {"url": "<img>", "detail": "low"},
...
def full_prompt_segment_sans_context(
    prompt_no: int,
    class_labels: list,
    shape: str,
    model: str,
    ...
    system_prompt = system_prompts_segment_sans_context(
        prompt_no, class_labels, shape, classes_guessed, description
    )
    messages.append({"role": "system", "content": system_prompt})
...
def full_prompt_naive(
    ...
    system_prompt = system_prompts_naive(
        prompt_no, class_labels, batch_size, classes_guessed
    )
    messages.append({"role": "system", "content": system_prompt})
...
def get_bbox_coords(segments: np.ndarray):
    ...
    bbox_coords = np.zeros(
        (n_formed_segments, 4), dtype=int
    )  # (n, 4) array to hold bbox coordinates
...
@TextMFMWrapper.register_task("segment")
def segment(
    ...
    for img_idx, img in enumerate(imgs):
        ...
        for start_idx in tqdm(
            range(1, n_unique_segments + 1, batch_size), disable=not display_pbar
        ):
            prompt_imgs = [img]
            for j in range(start_idx, start_idx + batch_size):
                if j > n_unique_segments:
                    break
                segment = draw_around_superpixel(
                    img, segments, j, shape, crop_width=1, radius=4
                )
            ...
            for j in range(start_idx, start_idx + batch_size):
                if j > n_unique_segments:
                    break
                segment_2 = draw_around_superpixel(
                    img, segments, j, shape, crop_width=40, radius=4
                )
            ...
            for k in range(batch_size):
                ...
                if resp_dict and isinstance(resp_dict, dict):
                    all_results[str(start_idx + k)] = resp_dict.get(
                        str(k + 1), "unknown"
                    )
                else:
                    all_results[str(start_idx + k)] = "unknown"

            classes_guessed += [
                v for k, v in resp_dict.items() if k != "reasoning_steps"
            ]
...
@TextMFMWrapper.register_task("segment_sans_context")
def segment_sans_context(
    ...
    Tuple[List[Dict], Tuple[int, int], bool], Tuple[List[np.ndarray], Tuple[int, int]]
]:
    ...
    def process_segment(img, segments, start_idx, img_idx):
        ...
        prompt_imgs = []
        segment = draw_around_superpixel(
            img, segments, start_idx, shape, crop_width=None
        )
        prompt_imgs.append(segment)

        full_prompt = (
            full_prompt_segment_sans_context(
                prompt_no,
                labels,
                shape,
                model.name,
    ...
    for img_idx, img in enumerate(imgs):
        ...
        with ThreadPoolExecutor(max_workers=1) as executor:
            futures = {
                executor.submit(
                    process_segment, img, segments, start_idx, img_idx
                ): start_idx
            ...
            for future in tqdm(as_completed(futures)):
                ...
                try:
                    ...
                    error_status = (
                        error_status or seg_error
                    )  # Set error_status if any segment failed
                except Exception as e:
                    print(
                        f"An error occurred while processing segment {start_idx} of image {img_idx}: {e}"
                    )
                    error_status = True
        ...
        all_results = {
            str(idx): result for idx, result in segment_results if result is not None
        }
        all_results["file_name"] = file_name[img_idx].strip()
...
@TextMFMWrapper.register_task("segment_naive")
def segment_naive(
    model: TextMFMWrapper,
    file_name: Union[List[str], str, List[Image.Image], Image.Image],
    prompt: Optional[Dict] = None,
    ...
    assert shape in [
        ...
        "rectangle",
        "none",
    ], "Invalid shape type. Choose from 'curve', 'rectangle', or 'none'."
    ...
    for img_idx, img in enumerate(imgs):
        ...
        mapping = (
            {i: j for i, j in zip(idxs, np.random.permutation(idxs))}
            if shuffle
            else {i: i for i in idxs}
        )
        ...
        for start_idx in tqdm(range(0, len(bboxes), batch_size)):
            ...
            annotated_image = label_annotator.annotate(
                ...
                labels=[str(j) for j in range(len(indices))],
            )
