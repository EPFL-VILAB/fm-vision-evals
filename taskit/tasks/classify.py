...
def system_prompts_cls(prompt_no: int, class_list: list, batch_size: int) -> str:
    ...
    if prompt_no == 1:
        system_prompt = (
            f"""You will be provided with a set of {n_classes} classes in a list. The user will provide you with {batch_size} images, """
        ...
        system_prompt = (
            f"""You are an AI assistant tasked with classifying images. You will be provided with {batch_size} images and must assign each image to one of the {n_classes} classes. Follow these instructions carefully:\n"""
        ...
        system_prompt = (
            f"""You are a highly accurate image classification AI. You will be provided with a comprehensive set of {n_classes} classes. The user will present you with {batch_size} images for classification. Your task is to analyze each image carefully and determine the most appropriate label from the given classes.\n\n"""
            + """For each image:\n"""
...
def system_prompts_cls_mult(class_list: list) -> str:
    system_prompt = (
        """You are an AI assistant tasked with identifying all the classes in images. You will be provided with an image and must identify all the classes present in the image. """
...
def system_prompt_cls_crop(class_list: list):
    system_prompt = (
        """You are an AI assistant tasked with identifying classes. You will be provided with a crop from an image and the full image the crop was taken from. You must first describe the cropped image in """
...
def json_schema_cls(bs: int, model: str):
    if model in OPENAI_MODELS:
        ...
        json_properties.update(
            {
                str(k + 1): {"type": "string", "description": f"class for image {k}."}
                for k in range(bs)
            }
        )
        ...
        json_schema = genai.protos.Schema(
            ...
            properties={
                str(k + 1): genai.protos.Schema(
                    type=genai.protos.Type.STRING, description=f"class for image {k}."
                )
                for k in range(bs)
            },
            required=[str(k + 1) for k in range(bs)],
    ...
    messages.append(
        {
            ...
            "content": [
                {
                    "type": "text",
                    "text": """Here is the image for which you need to identify all the classes.""",
                },
    ...
    messages.append(
        {
            ...
            "content": [
                {
                    "type": "text",
                    "text": """Here is the full image for some additional context.""",
                },
                {"type": "image_url", "image_url": {"url": "<img>", "detail": "high"}},
            ],
        }
    ...
    messages.append(
        {
            ...
            "content": [
                {
                    "type": "text",
                    "text": "Here is the cropped portion of the full image.",
                },
                {"type": "image_url", "image_url": {"url": "<img>", "detail": "high"}},
            ],
        }
...
@TextMFMWrapper.register_task("classify")
def classify(
    ...
    if error_status:
        resp_list = [
            {"class": "unknown", "file_name": file_name[i].strip()}
    ...
    for i in range(len(resp_dict)):
        resp_list.append(
            {"class": resp_dict[str(i + 1)], "file_name": file_name[i].strip()}
        )
...
@TextMFMWrapper.register_task("classify_mult")
def classify_mult(
    model: TextMFMWrapper,
    ...
    for img_idx, img in enumerate(imgs):
        full_prompt = (
            full_prompt_cls_mult(class_labels, model.name) if not prompt else prompt
        )
        ...
        resp_dict["classes"] = [
            cls for cls in resp_dict["classes"] if cls in class_labels
        ]  # remove classes that are not in the class list
        all_classes.append(
            {"class": resp_dict["classes"], "file_name": file_name[img_idx].strip()}
        )
...
@TextMFMWrapper.register_task("classify_crop")
def classify_crop(
    model: TextMFMWrapper,
    file_name: Union[List[str], str, List[Image.Image], Image.Image],
    prompt: Optional[Dict] = None,
    ...
    for img_idx, img in enumerate(imgs):
        ...
        # add the center crop
        cropped_images.append(
            img.crop((width // 4, height // 4, 3 * width // 4, 3 * height // 4))
        )
        pred_classes = set()
        for msg_idx in range(len(cropped_images)):
            full_prompt = (
                full_prompt_cls_crop(class_labels, model.name) if not prompt else prompt
            )
            full_prompt = replace_images_in_prompt(
                full_prompt, [img, cropped_images[msg_idx]]
            )
        ...
        pred_classes = [
            cls for cls in pred_classes if cls in class_labels
        ]  # remove classes that are not in the class list
        all_classes.append(
            {"class": list(pred_classes), "file_name": file_name[img_idx].strip()}
        )
