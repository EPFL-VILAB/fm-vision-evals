...
def system_prompts_normals(prompt_no: int, shape: str) -> str:
    ...
    if prompt_no == 1:
        ...
        system_prompt = (
            """You are an expert in computer vision and surface normal prediction. Your task is to analyze an image of an indoor scene and determine the orientation of two specific regions marked by colored shapes. This task requires a nuanced understanding of 3D space as represented in 2D images.\n\n"""
...
def json_schema_normals(prompt_no: int, model: str):
    if model in OPENAI_MODELS:
        if prompt_no in [1, 2]:
            ...
            json_properties["right"] = {
                "type": "string",
                "enum": ["red", "blue", "equal"],
                "description": "The color of the region that faces right (from the point of view of the viewer). If both are regions equally face right, output 'equal'. If a region faces left, output the color of the other region.",
            }
            json_properties["up"] = {
                "type": "string",
                "enum": ["red", "blue", "equal"],
                "description": "The color of the region that faces up. If both are regions equally face up, output 'equal'. If a region faces down, output the color of the other region.",
            }
            json_properties["out"] = {
                "type": "string",
                "enum": ["red", "blue", "equal"],
                "description": "The color of the region that faces out of the image. If both are regions equally face out of the image, output 'equal'. Note that a region facing inwards won't be visible to the camera. Therefore, output the color of the region that faces outwards, or output 'equal' if no region faces outwards.",
            }
...
@TextMFMWrapper.register_task("normals")
def normals(
    ...
    for img_idx, img in enumerate(imgs):
        ...
        def process_segment_pair(index, seg_pair):
            ...
            full_prompt = (
                full_prompt_normals(prompt_no, model.name, shape)
                if not prompt
                else prompt
            )
...
@ImageMFMWrapper.register_task("dense_normals")
def dense_normals(
    model: ImageMFMWrapper,
    file_name: Union[List[str], str, List[Image.Image], Image.Image],
    *,
    center_crop: bool = False,
    save_dir: str = "./4o-imagegen/normals-preds/",
    return_dict: bool = False,
):
    """
    Produces a per-pixel RGB normal map (standard CG color scheme) from an image.

    Args:
        model: The MFM model to use.
        file_name: Path(s) or PIL.Image(s) to run normal-map on.
        center_crop: if True, center-crop via crop_img before sending. Otherwise, image is padded to square (1024x1024)
        save_dir: where to save output.
        return_dict: whether to return a list-of-dicts or the final array.

    Returns:
        (if return_dict is True)
        outputs: List of dicts, each containing the "normal_image" and "file_name" keys.
        tokens: A tuple containing the completion tokens and the prompt tokens
        error_status: A boolean indicating whether an error occurred
        ...
        (if return_dict is False)
        normal_imgs: List of normal maps as np.ndarray (display using plt.imshow())
        tokens: A tuple containing the completion tokens and the prompt tokens
    """

    normal_prompt = """
    You are a vision model that, given an input RGB image, must predict a per-pixel surface normal map and render it as an RGB image using the standard normal-map color scheme.
    ...
    - Display the orientation of the surface normal vector using the standard color scheme used in computer graphics.
    """
    ...
    for fn in file_list:
        ...
        # process: either center-crop or pad/original
        if center_crop:
        ...
        # decode result
        if not err:
            normal_img = Image.open(BytesIO(base64.b64decode(resp_b64)))
            if not center_crop:
                normal_img = normal_img.crop((0, 128, 1024, 896))  # remove padding
        else:

            size = (crop_size, crop_size) if center_crop else (1024, 896)
            normal_img = Image.new("RGB", size, (0, 0, 0))

        #  in normal_img, invert the G and B channels
        normal_array = np.array(normal_img)
        normal_array[:, :, 1] = 255 - normal_array[:, :, 1]
        normal_array[:, :, 2] = 255 - normal_array[:, :, 2]
        normal_img = Image.fromarray(normal_array, "RGB")

        folder_name = fn.strip().split("/")[-2]
        out_path = os.path.join(
            save_dir, folder_name + "-" + os.path.basename(fn.strip())
        )
        normal_img.save(out_path)
        outputs.append({"normal_image": out_path, "file_name": fn.strip()})
        normal_imgs.append(normal_img)

    if return_dict:
        return outputs, (compl_tokens, prompt_tokens), error_status
    else:
        normal_imgs = [np.array(img, dtype=np.float32) / 255.0 for img in normal_imgs]
        return normal_imgs, (compl_tokens, prompt_tokens)
