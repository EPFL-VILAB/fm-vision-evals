...
def compute_correl_metrics(optimal_xs, average_norm, seg_map, gt_normals):
    pred_images = []
    gt_pixelated = []
    ...
    for opt_idx, optimal_x in enumerate(optimal_xs):
        if opt_idx == 1 or opt_idx == 2:
            optimal_x = -optimal_x
        optimal_x = (
            (optimal_x - optimal_x.min()) / (optimal_x.max() - optimal_x.min()) * 255
        )
    ...
    for i in range(3):
        ...
        local_correl_tau_direction = kendalltau(
            optimal_x_direction, average_norm[:, i]
        ).correlation
        local_correl_rho_direction = spearmanr(
            optimal_x_direction, average_norm[:, i]
        ).correlation
    ...
    for i in range(3):
        ...
        global_correl_tau_direction = kendalltau(
            pred_image.flatten(), norm_gts[i].flatten()
        ).correlation
        global_correl_rho_direction = spearmanr(
            pred_image.flatten(), norm_gts[i].flatten()
        ).correlation
...
def compute_dense_correl_metrics(normal_map, gt_normals):
    pred_images = [normal_map[:, :, 0], normal_map[:, :, 1], normal_map[:, :, 2]]
    norm_gts = [gt_normals[:, :, 0], gt_normals[:, :, 1], gt_normals[:, :, 2]]
...
def find_relative_normals(
    normal_orders_preds, seg_pairs, segment_map, smoothness_weight
):
    normal_orders = []
    ...
    normal_order_segment_map = {
        tuple(segment): normal_orders[i]
        for i, segment in enumerate(seg_pairs)
        if tuple(segment) not in normal_order_segment_map
    }
    normal_order_unique = np.array(
        [normal_order_segment_map[tuple(segment)] for segment in all_segments_unique]
    )
    ...
    for i in range(3):
        ...
        objective = cp.Minimize(
            ...
            + cp.quad_form(x, A_eq.T @ A_eq + eps_noise)
            + smoothness_weight
            * cp.quad_form(
...
@TextMFMWrapper.register_eval("eval_normals")
def eval_normals(
    ...
    """Returns several metrics for normals after reading outputs from 'predictions'
    ...
    Returns:
        ...
        avg_accuracy: percentage of correctly predicted pairwise rankings (to be deprecated)
    ...
    for direction in ["right", "up", "out"]:
        (
            avg_accuracy[direction],
            avg_local_kendall_tau[direction],
            avg_local_spearman_rank[direction],
            avg_global_kendall_tau[direction],
            avg_global_spearman_rank[direction],
        ) = ([], [], [], [], [])
    ...
    for output_dict in tqdm(outputs["data"], disable=visualise):
        ...
        optimal_xs = find_relative_normals(
            normal_orders_preds, seg_pairs, segments, smoothness_weight
        )
        ...
        if visualise:
            ...
            for opt_idx, optimal_x in enumerate(optimal_xs):
                if opt_idx == 1 or opt_idx == 2:
                    optimal_x = -optimal_x
                optimal_x = (
                    (optimal_x - optimal_x.min())
                    / (optimal_x.max() - optimal_x.min())
                    * 255
                )
                optimal_x = optimal_x.astype(int)
            ...
            normal_img = gt_normals[gt_counter]

            average_norm = np.zeros((n_points, 3))
            average_stds = np.zeros((n_points, 3))
            for point_idx in range(n_points):
                average_norm[point_idx] = np.mean(
                    normal_img[segments == (point_idx + 1)], axis=0
                )
                average_stds[point_idx] = np.std(
                    normal_img[segments == (point_idx + 1)], axis=0
                )

            for seg_idx, (seg1, seg2) in enumerate(seg_pairs):
                for dir_idx, direction in enumerate(["right", "up", "out"]):
                    if (normal_orders_preds[seg_idx][direction] == "equal") and (
                        np.abs(
                            average_norm[seg1 - 1, dir_idx]
                            - average_norm[seg2 - 1, dir_idx]
                        )
                        <= max(
                            average_stds[seg1 - 1, dir_idx],
                            average_stds[seg2 - 1, dir_idx],
                        )
                    ):
                        accuracies[direction].append(1)
                    elif (
                        (normal_orders_preds[seg_idx][direction] == "red")
                        and (
                            average_norm[seg1 - 1, dir_idx]
                            < average_norm[seg2 - 1, dir_idx]
                        )
                        or (normal_orders_preds[seg_idx][direction] == "blue")
                        and (
                            average_norm[seg1 - 1, dir_idx]
                            > average_norm[seg2 - 1, dir_idx]
                        )
            ...
            for direction in ["right", "up", "out"]:
                accuracies[direction] = sum(accuracies[direction]) / len(
                    accuracies[direction]
                )
            ...
            for direction in ["right", "up", "out"]:
                avg_local_kendall_tau[direction].append(
                    local_correl_tau[["right", "up", "out"].index(direction)]
                )
                avg_local_spearman_rank[direction].append(
                    local_correl_rho[["right", "up", "out"].index(direction)]
                )
                avg_global_kendall_tau[direction].append(
                    global_correl_tau[["right", "up", "out"].index(direction)]
                )
                avg_global_spearman_rank[direction].append(
                    global_correl_rho[["right", "up", "out"].index(direction)]
                )
        gt_counter += 1
    ...
    all_average_global_spearman_rank = list(
        (
            np.array(avg_global_spearman_rank["right"])
            + np.array(avg_global_spearman_rank["up"])
            + np.array(avg_global_spearman_rank["out"])
        )
        / 3
    )
    return_dict = {}
    return_dict["avg_accuracy"] = {
        direction: np.mean(avg_accuracy[direction])
        for direction in ["right", "up", "out"]
    }
    return_dict["avg_local_kendall_tau"] = {
        direction: np.mean(avg_local_kendall_tau[direction])
        for direction in ["right", "up", "out"]
    }
    return_dict["avg_local_spearman_rank"] = {
        direction: np.mean(avg_local_spearman_rank[direction])
        for direction in ["right", "up", "out"]
    }
    return_dict["avg_global_kendall_tau"] = {
        direction: np.mean(avg_global_kendall_tau[direction])
        for direction in ["right", "up", "out"]
    }
    return_dict["avg_global_spearman_rank"] = {
        direction: np.mean(avg_global_spearman_rank[direction])
        for direction in ["right", "up", "out"]
    }
    return_dict["all_average_global_spearman_rank"] = all_average_global_spearman_rank

    return return_dict


@ImageMFMWrapper.register_eval("eval_dense_normals")
def eval_dense_normals(
    predictions: Union[List, str],
    ground_truth: str = None,
    invalid_files: list = [],
    read_from_file: bool = False,
    ...
    """Returns Kendall's tau and Spearman's rank correlation for normals after reading outputs from 'predictions'. An evaluation function for dense predictions.
    ...
    Returns:
    (metrics after superpixelation, for consistency with eval_normals)
    avg_accuracy: percentage of correctly predicted pairwise rankings (to be deprecated)
    avg_local_kendall_tau: float, average local Kendall's tau  (local: ranking correlation of the superpixels wrt ground-truth superpixels)
    avg_local_spearman_rank: float, average local Spearman's rank correlation (ranking of superpixels)
    avg_global_kendall_tau: float, average global Kendall's tau (global: ranking correlation of all the pixels wrt ground-truth pixels)
    ...
    avg_dense_kendall_tau: float, Dense Global Kendall's tau correlation
    avg_dense_spearman_rank: float, Dense Global Spearman's rank correlation

    All the metrics are computed per direction.
    """
    ...
    # --Form the predicted images------------------------------------------------
    file_name_to_normal_image = {
        dic["file_name"]: dic["normal_image"] for dic in outputs["data"]
    }
    normal_maps = [
        np.array(Image.open(file_name_to_normal_image[file_name]).convert("RGB"))
    ...
    for direction in ["right", "up", "out"]:
        (
            ...
            avg_global_kendall_tau[direction],
            avg_global_spearman_rank[direction],
            avg_dense_kendall_tau[direction],
            avg_dense_spearman_rank[direction],
        ) = ([], [], [], [], [], [], [])
    ...
    for output_dict in tqdm(outputs["data"]):
        ...
        for dir_idx, direction in enumerate(["right", "up", "out"]):
            optimal_x = []
            for _, segment in enumerate(np.unique(segments)):
                optimal_x_value = np.mean(
                    normal_maps[gt_counter][segments == segment][:, dir_idx]
                )
                if dir_idx == 1 or dir_idx == 2:
                    optimal_x_value = -optimal_x_value
                optimal_x.append(optimal_x_value)
            optimal_xs.append(np.array(optimal_x))
        ...
        average_norm = np.zeros((n_points, 3))
        average_stds = np.zeros((n_points, 3))
        average_pred_norm = np.zeros((n_points, 3))
        average_pred_stds = np.zeros((n_points, 3))
        for point_idx in range(n_points):
            average_norm[point_idx] = np.mean(
                normal_img[segments == (point_idx + 1)], axis=0
            )
            ...
            average_pred_norm[point_idx] = np.mean(
                normal_maps[gt_counter][segments == (point_idx + 1)], axis=0
            )
            average_pred_stds[point_idx] = np.std(
                normal_maps[gt_counter][segments == (point_idx + 1)], axis=0
            )
        ...
        for seg_idx, (seg1, seg2) in enumerate(seg_pairs):
            for dir_idx, direction in enumerate(["right", "up", "out"]):
                if (
                    np.abs(
                        average_pred_norm[seg1 - 1, dir_idx]
                        - average_pred_norm[seg2 - 1, dir_idx]
                    )
                    <= max(
                        average_pred_stds[seg1 - 1, dir_idx],
                        average_pred_stds[seg2 - 1, dir_idx],
                    )
                ) and (
                    np.abs(
                        average_norm[seg1 - 1, dir_idx]
        ...
        dense_global_correl_tau, dense_global_correl_rho = compute_dense_correl_metrics(
            normal_maps[gt_counter], normal_img
        )
        for direction in ["right", "up", "out"]:
            avg_local_kendall_tau[direction].append(
                local_correl_tau[["right", "up", "out"].index(direction)]
            ...
            avg_dense_kendall_tau[direction].append(
                dense_global_correl_tau[["right", "up", "out"].index(direction)]
            )
            avg_dense_spearman_rank[direction].append(
                dense_global_correl_rho[["right", "up", "out"].index(direction)]
            )
        gt_counter += 1
    ...
    return_dict["avg_dense_kendall_tau"] = {
        direction: np.mean(avg_dense_kendall_tau[direction])
        for direction in ["right", "up", "out"]
    }
    return_dict["avg_dense_spearman_rank"] = {
        direction: np.mean(avg_dense_spearman_rank[direction])
        for direction in ["right", "up", "out"]
    }
