...
from taskit.utils.data_constants import (
    COCO_SEMSEG_LABELS,
    COCO_COLOR_MAP,
    COCO_LABEL_2_ID,
)
...
def process_single_image(args):
    ...
    pred_array = np.array(
        flood_superpixels(img, all_pred, label_to_id, n_segments, ignore_index)
    )
    if visualise:
...
@TextMFMWrapper.register_eval("eval_segment")
def eval_segment(
    ...
    for label in labels:
        if label not in color_map:
            print(
                f"Warning: {label} not present in color map. Adding with black color."
            )
    ...
    if not visualise:
        ...
        for file_idx, file_name in enumerate(
            [output["file_name"] for output in outputs["data"]]
        ):
            if file_name in rgb_data_files:
    ...
    for output_dict in outputs["data"]:
        ...
        for j, (k, v) in enumerate(output_dict.items()):
            if k != "file_name" and k != "coordinates":
                preds.append(v)
        all_preds.append(preds)

    n_images = len(rgb_data_files)
