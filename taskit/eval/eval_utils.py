...
def get_map(
    ...
    groundtruth = json.load(open('./taskit/utils/metadata/coco-detect.json'))  # json file, in the same format as coco/annotations/instances_val2017.json
    category_id_2_name = {ann['id']: ann['name'] for ann in groundtruth['categories']}
    name_2_category_id = {name: category_id for category_id, name in category_id_2_name.items()}
    ...
    for i, image_file in tqdm(enumerate(rgb_data_files)):
        ...
        image_dict = {"id": image_id, "file_name": file_name, "width": width, "height": height}  # Add the image to the COCO dataset
        ...
        for pred_bbox_idx, (name, bbox) in enumerate(pred_dict.items()):
            ...
            ann_dict = {"image_id": image_id, "category_id": category_id, "bbox": [x1, y1, width, height], "score": scores[pred_bbox_idx]}
    ...
    map_01, map_05, map_075, map_05_095 = results[0], results[1], results[6], np.mean(results[1:])
